clear; clc;
% =========================================================================
% 园区综合能源系统两阶段鲁棒多目标优化 (MO-TSRO) 终极版
% 核心架构：ε-约束法 + C&CG + CRITIC-TOPSIS 客观决策 + SCI级全套可视化
% =========================================================================
...
Archive     = cell(n_pareto+2, 1); % 保存每个点的最优调度变量
...
%% =========================================================================
%  第2节：CRITIC-TOPSIS 客观多属性决策 (评价提取最优折衷解)
% =========================================================================
valid_mask = ~isnan(pareto_f1) & ~isnan(pareto_f2);
idx_list = find(valid_mask);
f1_valid = pareto_f1(valid_mask);
f2_valid = pareto_f2(valid_mask);

% 组建评价矩阵 X (n个样本，m=2个指标)
X = [f1_valid, f2_valid];
[n_points, m_objs] = size(X);

% 步骤 2.1: 正向化与标准化 (越小越优 -> 反向极差标准化)
X_norm = zeros(n_points, m_objs);
for j = 1:m_objs
...
R = corrcoef(X_norm);        
C_info = S .* sum(1 - R, 2)';
W_critic = C_info ./ sum(C_info); 

fprintf('\n▶ [CRITIC 客观赋权结果] f1(成本)权重: %.4f, f2(不满度)权重: %.4f\n', W_critic(1), W_critic(2));

% 步骤 2.3: TOPSIS 逼近理想解排序
Z = X_norm .* W_critic;      
Z_plus = max(Z);             
Z_minus = min(Z);            

D_plus = sqrt(sum((Z - repmat(Z_plus, n_points, 1)).^2, 2));  
D_minus = sqrt(sum((Z - repmat(Z_minus, n_points, 1)).^2, 2)); 
Score_TOPSIS = D_minus ./ (D_plus + D_minus);

% 步骤 2.4: 提取最优折衷解
[~, best_rel_idx] = max(Score_TOPSIS);
best_abs_idx = idx_list(best_rel_idx);
best_arc = Archive{best_abs_idx};

fprintf('\n=============================================================\n');
fprintf(' 🏅 CRITIC-TOPSIS 决策完成！最优折衷点为第 %d 个有效点：\n', best_rel_idx);
fprintf('    综合贴近度 C_i = %.4f\n', Score_TOPSIS(best_rel_idx));
fprintf('    最优成本 f1 = %.2f 元\n', f1_valid(best_rel_idx));
fprintf('    最优不满度 f2 = %.4f\n', f2_valid(best_rel_idx));
fprintf('=============================================================\n');

%% =========================================================================
%  第3节：提取最优折衷点数据并进行最后一次确定性物理核算 
% =========================================================================
...
% =========================================================================
t = 1:24;
c_blue=[0.00 0.45 0.74]; c_orange=[0.85 0.33 0.10]; c_green=[0.47 0.67 0.19]; 
c_red=[0.85 0.10 0.10]; c_purple=[0.49 0.18 0.56]; c_gray=[0.5 0.5 0.5]; c_yellow=[0.95 0.85 0.25]; c_cyan=[0.30 0.75 0.93];

% -------- [图1] 带有 CRITIC-TOPSIS 最优点的帕累托前沿 --------
[pf2_sorted, sort_idx] = sort(f2_valid);
pf1_sorted = f1_valid(sort_idx);
% 寻找最优折衷点在排序后数组中的位置
best_idx_in_sorted = find(pf1_sorted == f1_valid(best_rel_idx) & pf2_sorted == f2_valid(best_rel_idx));

figure('Color','w','Position',[50,50,850,600],'Name','帕累托前沿与最优折衷点');
hold on; grid on; box on;
plot(pf2_sorted, pf1_sorted/1e4, '-', 'Color', c_gray, 'LineWidth', 2); % 连线
scatter(pf2_sorted, pf1_sorted/1e4, 120, 'filled', 'MarkerFaceColor', c_blue, 'MarkerEdgeColor', 'w', 'LineWidth', 1.5); % 所有点
% 突出显示 TOPSIS 最优折衷解 (画个大星星)
scatter(f2_valid(best_rel_idx), f1_valid(best_rel_idx)/1e4, 500, 'p', 'filled', 'MarkerFaceColor', c_orange, 'MarkerEdgeColor', 'k', 'LineWidth', 1.5);

for i = 1:length(pf2_sorted)
    if i == best_idx_in_sorted
        text(pf2_sorted(i)+0.003, pf1_sorted(i)/1e4+0.05, 'TOPSIS 最优折衷解', 'FontSize', 12, 'FontWeight', 'bold', 'Color', c_orange);
    else
        text(pf2_sorted(i)+0.002, pf1_sorted(i)/1e4+0.03, sprintf('P%d', i), 'FontSize', 10, 'Color', c_blue);
    end
end
xlabel('f_2：用户体验恶化度 (需求响应灵活性指数)', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('f_1：系统综合运行成本 / 万元', 'FontSize', 14, 'FontWeight', 'bold');
title('MO-TSRO 帕累托前沿与 CRITIC-TOPSIS 决策最优解', 'FontSize', 16, 'FontWeight', 'bold');
legend('Pareto 前沿', '非支配解 (候选方案)', 'CRITIC-TOPSIS 最优折衷解', 'Location', 'northeast', 'FontSize', 12);

% -------- [图2] 全系统单日精细化功率平衡与温控调度图 --------
figure('Color','w','Position',[100, 100, 1200, 800],'Name','最优解精细调度全景图');
subplot(2,2,1);
Supply_E = [P_grid, P_pv, P_wind, P_ES_dis, P_HFC_e]; Consume_E = [-P_ES_ch, -P_EC, -P_EL, -P_EB];
...
title('最优折衷点：电功率平衡', 'FontSize', 12, 'FontWeight', 'bold'); xlabel('时间/h'); ylabel('功率/kW'); xlim([0.5 24.5]); xticks(2:2:24); grid on;
...
title('最优折衷点：热功率平衡', 'FontSize', 12, 'FontWeight', 'bold'); xlabel('时间/h'); ylabel('功率/kW'); xlim([0.5 24.5]); xticks(2:2:24); grid on;
...
title('最优折衷点：冷功率平衡', 'FontSize', 12, 'FontWeight', 'bold'); xlabel('时间/h'); ylabel('功率/kW'); xlim([0.5 24.5]); xticks(2:2:24); grid on;

subplot(2,2,4);
yyaxis left; plot(t, ES / V_ES, '-s', 'LineWidth', 1.5, 'Color', '#0072BD', 'MarkerFaceColor', 'w'); ylabel('电储能 SOC', 'Color', '#0072BD'); ylim([0 1]); set(gca, 'YColor', '#0072BD');
yyaxis right; plot(t, T_in, '-d', 'LineWidth', 1.5, 'Color', '#D95319', 'MarkerFaceColor', 'w'); ylabel('机房温度 / °C', 'Color', '#D95319'); ylim([15 30]); set(gca, 'YColor', '#D95319');
yline(27.22, '--r'); yline(17.78, '--b');
title('最优折衷点：电储能SOC变化与数据中心精细温控', 'FontSize', 12, 'FontWeight', 'bold'); xlabel('时间/h'); xlim([0.5 24.5]); xticks(2:2:24); grid on;
...
legend('预测负荷', '响应负荷', '转入', '转出', '削减', 'Location','southoutside','NumColumns',2);
...
ylabel('DC算力功率 / kW'); ylim([min(Load_DC)*0.7, max(Load_DC)*1.2]); ax = gca; ax.YColor = 'k';
title('最优折衷点：DC延迟响应', 'FontSize', 12); xlabel('时间 / h'); xlim([1 24]); grid on;
legend('转入', '转出', '基础算力', '优化算力', 'Location','southoutside','NumColumns',2);
