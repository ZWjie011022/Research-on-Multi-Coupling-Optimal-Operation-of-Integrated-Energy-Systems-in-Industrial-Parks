clear; clc;
% =========================================================================
% 园区综合能源系统 4典型日 全年容量优化配置模型（无风光、无碳对照场景）
% 【HFC灵活电热比版】
% 列 1: 春季 (91天)  列 2: 夏季 (91天)
% 列 3: 秋季 (91天)  列 4: 冬季 (92天)
% 
% 【核心逻辑】：
% 1. 优化完全不考虑碳排放与碳交易成本，纯经济驱动。
% 2. 引入真实风光出力数据，但强制限制风光装机容量为0，作为严格的对照组。
% 3. 氢燃料电池（HFC）电热效率之和固定为0.9，电热比在[0.25, 4]范围内灵活调节。
% =========================================================================
...
% 由于 V_pv 和 V_wind 已锁定为0，此处的最大出力限制必然全为0
...
for d = 1:4
    ...
    constraint = [constraint, ...
        ...
        P_EB(:,d)*yita_EB + P_HFC_h(:,d) >= Load_H(:,d) + P_AC_h(:,d), ... % HFC供热
        P_EL(:,d)*yita_EL   == P_HS_ch(:,d), ...
        P_grid(:,d) + P_pv(:,d) + P_wind(:,d) - P_ES_ch(:,d) + P_ES_dis(:,d) - P_EC(:,d) - P_EL(:,d) + P_HFC_e(:,d) - P_EB(:,d) >= Load_E(:,d) + Load_DC(:,d), ... % HFC供电
...
if sol.problem ~= 0
    ...
    disp('===================================================');
    disp('恭喜，[纯经济驱动-无风光常规调度场景-HFC灵活版] 优化成功求解！');
    disp('===================================================');
    fprintf('\n[最优装机容量]\n');
...
title('电储能 SOC 与 数据中心温控 四季动作协同 (无风光对照组)', 'FontSize', 14);
