clear; clc;
% =========================================================================
%对照组，主要是对比场景三和场景2的,实际上就是对比场景三和场景二，当场景三风光出力锁定后，场景二定好计划后需要付出多少成本
%才能在日内平衡
% =========================================================================
...
%% =========================================================================
% [步骤二] 载入并锁死第一阶段决策变量
% 运行位置：在场景二的变量声明之后，约束条件定义的过程中
% =========================================================================

% 1. 载入刚才保存的固定数据
if isfile('Fixed_DR_Vars.mat')
    load('Fixed_DR_Vars.mat');
    disp('📦 成功载入固化的第一阶段决策变量！');
else
    error('未找到 Fixed_S3_Vars.mat 文件，请先运行提取代码！');
end

% 2. 将场景二的决策变量"锁死" (添加严格的等式约束)
% 这样求解器在跑场景二时，这些变量就变成了已知常数，只优化剩下的设备出力
Lock_Constraints = [
    % 锁死 IDR 电变量
    P_E_sl_in == fix_P_E_sl_in, P_E_sl_out == fix_P_E_sl_out, P_E_cut == fix_P_E_cut,
    % 锁死 IDR 热变量
    P_H_sl_in == fix_P_H_sl_in, P_H_sl_out == fix_P_H_sl_out, P_H_cut == fix_P_H_cut,
    % 锁死 IDR 冷变量
    P_C_sl_in == fix_P_C_sl_in, P_C_sl_out == fix_P_C_sl_out, P_C_cut == fix_P_C_cut,
    
    % 锁死 DC 变量
    P_DC_delay1 == fix_P_DC_delay1, P_DC_delay2 == fix_P_DC_delay2,
    Load_DC_opt == fix_Load_DC_opt,
    
    % 锁死储能充放电状态 (因为二进制变量在 DRO 里已经决定好了)
    U_ES_char == fix_U_ES_char,
    U_HS_char == fix_U_HS_char
];

% 3. 将锁死约束合并到你场景二原有的总约束中
C = [C, Lock_Constraints];

% ---> 接下来你就可以正常调用 sol = optimize(Constraints, Objective, ops) 了 <---
disp('🔒 变量已锁定！准备开始场景二的二次优化求解...');
